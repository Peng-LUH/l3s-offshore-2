...
class OffshoreSimulator(TimedSimulator):
    ...
    def __init__(self, timed_petri_net: TimedPetriNet, properties=None, start_at=None, stop_at=None):
        super().__init__(timed_petri_net, properties)
        
        self._template_net = deepcopy(timed_petri_net)
        
        self.__start_at = start_at if start_at is not None else 0
        self.__stop_at = stop_at if stop_at is not None else 480
        self.reset()
